...
test_that("hookregNL gives the correct dimensions and properties", {
  ...
  expect_that(round(res_hookregNL_positve[["slope"]], 8) == -0.01596799, expect_true())
  expect_that(res_hookregNL_positve_complex, is_a("list"))
  expect_that(res_hookregNL_negative, is_a("data.frame"))
  expect_that(res_hookregNL_positve[["hook"]] == 1, expect_true())
  expect_that(res_hookregNL_positve_complex[["hook"]] == 1, expect_true())
  expect_that(res_hookregNL_positve_complex[["fit"]][["weights"]][1] == 1, expect_true())
  expect_that(round(res_hookregNL_positve_complex[["slope"]], 8) == -0.01596799, expect_true())
  expect_that(res_hookregNL_negative[["hook"]] == 1, expect_false())
})
