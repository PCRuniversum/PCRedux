#' winklR: A function to calculate the angle based on the first and the second #' derivative of an amplification curve data
...
#'
#' # Plot the data
#' plot(RAS002[, 1], y=RAS002[, 2], xlab="Cycle", ylab="RFU",
...
#'
#' plot(RAS002[, 1], y=RAS002[, 7], xlab="Cycle", ylab="RFU",
#'      main="RAS002 data set", lty=1, type="l")
#' res <- winklR(x=RAS002[, 1], y=RAS002[, 7])
#' res
#' plot(rbind(res$origin, res$p1, res$p2), col = c("black","green","blue"))
#' 
#' res_angles <- unlist(lapply(2:ncol(RAS002), function(i) {winklR(RAS002[, 1], RAS002[, i])$angle}))
#' cdplot(as.factor(RAS002_decisions) ~ res_angles, xlab = "angle", ylab = "decision")
#' 
#' 
#' @export winklR
...
winklR <- function(x, y, normalize = FALSE, preprocess = TRUE) {
  ...
  res <- try(suppressMessages(MBmca::diffQ2(cbind(x[-c(1:10)], y[-c(1:10)]), 
    ...
    u <- data.frame(u_x = origin[1] - point_x1[1], 
                    u_y = origin[2] - point_x1[2])
    
    v <- data.frame(v_x = origin[1] - point_x2[1], 
                    V_y = origin[2] - point_x2[2])

    
    # Determin the dot product and the absolute values
    dot_product <- sum(u * v)
