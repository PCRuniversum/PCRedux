...
### Classified Amplification Curves\label{chapter_humanrater}

Amplification curves from different sources (e.g., detection chemistries, thermo-cyclers) were manually classified with the ``humanrater()`` function, as described in @roediger2015chippcr or with the ``tReem()`` function from the \texttt{PCRedux} package. The \autoref{chapter_humanrater} describes approaches to classify amplification curves.

Raw amplification curve data were exported as comma separated values or via the \texttt{RDML} package in the Real-time PCR Data Markup Language (RDML) format. RDML is human readable data exchange format for qPCR experiments. A detailed description can be found in @roediger_enabling_2017. The following code section describes the import of an RDML file from the \texttt{PCRedux} package. The RDML file contains amplification curve data of a duplex qPCR (HPV 16 & HPV 18) performed in the CFX96 (Bio-Rad).
...
\begin{table}[]
\centering
\caption{Classified amplification curve datasets. Decision Datasets in \texttt{PCRedux}: table with results of manual classification as comma separated values. qPCR Dataset: name of original amplification curve data set. Package: name of the \texttt{R} package containing the amplification curves. Device: is the device used to measure the amplification reaction. \texttt{Note: The original data sets contain information about the detection chemistry used within the corresponding qPCR experiments.} AB, Applied Biosystems.}
