...
- `cpD2_range` is the is the absolute value of the difference between the minimum and the maximum of the second derivative maximum ($cpD2_range = |\min cpD2 - \max cpD2|$) from the ``diffQ2()`` function (no model fitted) [@roediger2015chippcr]. The `cpD2_range` value does not require an adjustment of a multiparametric model. The approximate first and second derivatives are determined using a five-point stencil [@roediger2015chippcr]. To take different numbers of PCR cycles into account, the `cpD2_range` value is normalized to the total number of cycles. The comparison of positive and negative amplification curves in \autoref{plot_dat_EffTop}E shows that the classes differ significantly from each other. In the event that the `cpD2_range` value cannot be determined (NA), it is replaced by zero.
...
```{r, echo=FALSE, fig.cap="\\label{plot_bg_pt}A) b_slope, B) f\\_intercept, C) minRFU, D) maxRFU, E) init2, F) fluo, G) slope\\_bg, H) intercept\\_bg, I) sd\\_bg, J) head2tail_ratio.", fig.height=11, fig.width=11}
library(PCRedux)
