...
On the basis of this observation, various concepts were developed and implemented in algorithms to describe amplification curves. The function described following are aimed for experimental studies. It is important to note that the *concepts for the predictors* proposed herein emerged by a critical reasoning process. The aim of the package is to propose a set of predictors, functions and data for an independent research.
...
In the following example, the ``randomForest()`` [\texttt{randomForest}] function [@liaw_classification_2002] was used for the classification. The aim was to find the most important predictors for the classification of positive and negative amplification curves. 
As response vector ($y$) served `decision` with its possible finite classes labeled as "positive" and "negative". With classification problems we try to predict a discrete number of values. Thus in this case it is a binary classification. The response vector `decision` must be a vector.
...
test_data <- data[index_training, ]

# The randomForest() function was used to train a forest of 300 trees.
# Convert decision into factor, since randomForest() uses type="regression", 
# instead of type="classification" by default.


model_rf <- randomForest(as.factor(decision) ~ ., data = data, subset = index_training, 
...
The variables %IncMSE and IncNodePurity are used to determine the importance of the variables from a `Random Forest` model. %IncMSE is a robust quantity that indicates the increase in Mean Squared Error (MSE) when the specified variable is randomly permuted (estimated with out-of-bag CV). The higher the %IncMSE value of a variable, the more important it is. IncNodePurity refers to the loss function selected by best splits. More useful variables achieve a higher increase in node purities. This results in a split with a high inter node variance and a low intra node variance. The number of variables randomly selected at each split is `r model_rf$mtry`.
...
# Show the predictors sorted by their importance using %IncMSE and IncNodePurity

barplot(t(as.matrix(sort(res_importance[, 1]))), 
        ylab = "%IncMSE", ylim = c(0, max(res_importance[, 1]) + 5), main = "", 
        las = 2, col = adjustcolor("grey", alpha.f = 0.5), border = "white")
mtext("B", cex = 1, side = 3, adj = 0, font = 2, las = 0)

barplot(t(as.matrix(sort(res_importance[, 2]))), 
        ylab = "IncNodePurity", ylim = c(0, max(res_importance[, 2]) + 10), 
        main = "", las = 2, col = adjustcolor("grey", alpha.f = 0.5), 
        border = "white")
mtext("C", cex = 1, side = 3, adj = 0, font = 2, las = 0)
```

The top three important predictors are `shapiro` (`r res <- signif(res_importance[rownames(res_importance) == "shapiro"], 4); paste0("%IncMSE = ", res[1],  ", IncNodePurity = ", res[2])`), `polygon` (`r res <- signif(res_importance[rownames(res_importance) == "polygon"], 4); paste0("%IncMSE = ", res[1],  ", IncNodePurity = ", res[2])`) and `slt` (`r res <- signif(res_importance[rownames(res_importance) == "slt"], 4); paste0("%IncMSE = ", res[1],  ", IncNodePurity = ", res[2])`).
