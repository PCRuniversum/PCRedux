...
Data preparation is an important step. This includes the steps involved in data cleansing, data transformation and data integration[@herrera_multiple_2016]. The \texttt{xray} package [@Seibelt_xray] was used to analyze the distribution form and variables in records for anomalies such as missing values, zeros, infinite values and their categories. The ``anomalies()`` function from the \texttt{xray} package can be used to search for anomalies (including missing values (NA), zero values (Zero), blank strings (Blank) and infinite numbers (Inf)). The results of all  analysis are reported in columns prefixed by *q* for quantity and *p* for percentage. The later indicates the percent of total rows with the anomaly. Here the `decision_res_htPCR.csv` data set was analyzed to get an overview about a fairly large data set from `r ncol(qpcR:: htPCR)-1` amplification curves. This data set contains the sample names, eight columns of classifications (`test.result.1` $\ldots$ `test.result.8`) and a column about the conformity of all classifications.

```{r, echo=TRUE, fig.cap="\\label{figure_xray} Cleansing of data.", fig.height=8}
# Suppress messages and load the packages for reading the data and
# the analysis of anomalies.
options(warn = -1)
...
# Analyze the data set for missing values, zero values, blank strings and 
# infinite numbers.
anomalies(decision_htPCR, anomaly_threshold = 0.5, distinct_threshold = 1)
```

The results indicate the the *conformity* column contains in 94.54% identical values, with two distinct categories (*qDistinct*). There are no missing values, no zero values, blank strings and no infinite numbers in this data set.
...
### Classified Amplification Curve Data Sets \label{chapter_humanrater}
...
The `RAS002.rdml` and `RAS003.rdml` data sets were exported from the RDML format to the csv format and merged into a single file called `RAS002_RAS003_amp.csv`. In addition, the the decisions be the human expert were analyzed for the most frequent decision (modus) using the ``decision_modus()`` function (\autoref{section_decision_modus}).
...
RAS003 <- data.frame(RDML$new(paste0(path.package("PCRedux"), "/", "RAS003.rdml"))$GetFData())

# Bind the the RAS002 and RAS003 objects as object dat.
# Note: From the object RAS003 the first column, which contains the cycle 
# numbers was removed to avid  duplication.
dat <- cbind(RAS002, RAS003[, -1])

# The obbject dat was finally stored in the working directory as CSV file with 
# the name RAS002_RAS003_amp.csv.
write.csv(dat, "RAS002_RAS003_amp.csv", row.names=FALSE)


# Helper function to determine the modus from all decisions from the 
# decision_res_RAS002.csv and decision_res_RAS003.csv files.
decider <- function(name) {
    filename <- system.file(name, package = "PCRedux")
    data_tmp <- fread(filename, data.table = FALSE)
    data_tmp[data_tmp == "a"] <- "n"
    sapply(1L:nrow(data_tmp), function(i) {
        res <- decision_modus(data_tmp[i, 2L:(ncol(data_tmp)-1)])
        if(length(res) > 1) res[[1]] <- "n"
            res[[1]]
    }) %>% unlist()
}

get_current_wd <- getwd()

setwd(paste0(path.package("PCRedux"), "/"))
...
all_decision_test <- unlist(sapply(1L:length(decision_files), function(i) {
    decider(decision_files[i])
}
))

decs <- as.factor(c(all_decision_test[, 1], all_decision_test[, 2]))

setwd(get_current_wd)

# The obbject dat was finally stored in the working directory as CSV file with 
# the name RAS002_RAS003_decs.csv.
write.csv(decs, "RAS002_RAS003_decs.csv", row.names=FALSE)
```
...
```{r, echo=TRUE, eval=TRUE, fig.cap="\\label{figure_RAS002} RAS003 data set.", fig.height=4}
options(warn = -1)
library(PCRedux)
library(RDML)
RAS002 <- data.frame(RDML$new(paste0(path.package("PCRedux"), "/", "RAS002.rdml"))$GetFData())
colors <- rainbow(ncol(RAS002) -1, alpha = 0.3)
matplot(RAS002[, 1], RAS002[, -1], xlab = "Cycle", ylab = "RFU", 
...
```{r, echo=TRUE, fig.cap="\\label{figure_autocorrelation_tay} Effect of tau.", fig.height=8}
library(PCRedux)
amp_data <- fread(system.file("RAS002_RAS003_amp.csv", package = "PCRedux"), 
                  data.table = FALSE)
...
plot(amp_data[, 1], amp_data[, 10])
mtext("A", cex = 1.5, side = 3, adj = 0, font = 2)

ac_pos <- sapply(1L:length(tau), function(i) {
      autocorrelation_test(amp_data[, 10], n=tau[i])
  })
    
ac_pos == min(ac_pos)

plot(tau, ac_pos, ylim=c(0,1), pch=19)
points(tau[ac_pos == min(ac_pos)], ac_pos[ac_pos == min(ac_pos)], pch=1, cex=2, col="red")
legend("bottomleft", paste("tau: ",  tau[ac_pos == min(ac_pos)], "\nAC: ", signif(ac_pos[ac_pos == min(ac_pos)])), bty="n")
mtext("B", cex = 1.5, side = 3, adj = 0, font = 2)

plot(amp_data[, 1], amp_data[, 15])
mtext("C", cex = 1.5, side = 3, adj = 0, font = 2)
...
Note: The amplification curve data and the decision data were taken from the `RAS002_RAS003_amp.csv` and the `RAS002_RAS003_decs.csv` files. The preparation of the data is described in \autoref{chapter_humanrater}.

```{r, echo=TRUE, fig.cap="\\label{autocorrelation}Autocorrelation analysis for amplification curves of the `RAS002` and `RAS003` data sets (\\texttt{PCRedux} package). A) All amplification curves of the `RAS002` and `RAS003` data sets. B) Positive curves and negative curves as determined by the `autocorrelation_test()` and a human. C) Performance analysis by the ``performeR()`` function (see \\autoref{section_performeR} for details).", fig.height=5.5, fig.width=11}
...
# Note: The amplification curve data from the `RAS002` and `RAS003` data sets
# were exported from the RDML format to the CSV format and combined in a single
# file called RAS002_RAS003_amp.csv
...
# Test for autocorrelation in the RAS002 and RAS003 data sets
res_ac <- sapply(2:ncol(amp_data), function(i) {
    autocorrelation_test(amp_data[, i], ns_2_numeric=TRUE)
})

# Curves classified by a human after analysis of the overview. 1=positive,
# 0=negative

human_classification <- fread(system.file("RAS002_RAS003_decs.csv", package = "PCRedux"), 
                       data.table = FALSE) %>% unlist
...
matplot(
  amp_data[, 1], amp_data[, -1], xlab = "Cycle", ylab = "RFU",
  main = "RAS002 and RAS003 data sets", type = "l", lty = 1, col = as.factor(human_classification), lwd = 2
)
legend("topleft", c("negative", "positive"), pch = 19, col = c(1,2), bty = "n")
mtext("A", cex = 1.5, side = 3, adj = 0, font = 2)
...
res_ac_hc <- data.frame(
  ac = ifelse(res_ac > 0.8, 0, 1),
  hc = as.numeric(as.factor(human_classification))-1
) %>% as.matrix()
...
par(las = 2)
# plot(
#   1:nrow(res_ac_hc), res_ac_hc[, "hc"], xlab = "Sample",
#   ylab = "Decisions", xaxt = "n", yaxt = "n", pch = 19
# )
# axis(2, at = c(0, 1), labels = c("negative", "positive"), las = 2)
# axis(1, at = 1:nrow(res_ac_hc), labels = colnames(amp_data)[-1], las = 2)
# points(
#   1:nrow(res_ac_hc), res_ac_hc[, "ac"], pch = 1, cex = 2,
#   col = "red"
# )
# legend(
...
cdplot(as.factor(human_classification) ~ res_ac, xlab="Autocorrelation", ylab="Decision")
...
print(xtable(
  res_hook, caption = "Screening results for the analysis with the 
             ...
             these amplification curves did not have a hook effect like curvature.",
  label = "tablehookreg"
), include.rownames = FALSE, comment = FALSE, caption.placement = "top", size = "\\tiny")
```
