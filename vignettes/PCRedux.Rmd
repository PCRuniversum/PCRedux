...
## Analysis of Simgmoid Shaped Curves for Data Mining and Machine Learning Applications

In the following chapters information is provided, which can be used for the analysis and description of quantitative real-time PCR amplification curves. This work aims to provide technologies for the analysis and numerical description of sigmoid curves in order to make them accessible for further applications such as data mining and machine learning. The purpose of the \texttt{PCRedux} package is to enable automatic classification of positive and negative amplification curves. Admittedly, one may argue that such a classification is also feasible using other analytical methods such as melting curve analysis or an electrophoretical analysis. This is not always possible or desirable. For example, no melting curves cannot be obtained with Taqman probes. An electrophoretical method often requires too much effort for qPCR with high sample throughput. But this is not necessarily the target audience of the \texttt{PCRedux} package. Instead, it is aimed at users and developers who wish to classify amplification curves automatically. Apart from positive and negative, other classifications are conceivable.
...
The shape of a positive amplification curve follows in most cases a sigmoid shape. The curvature of the amplification curve can be used as a quality measure. For example, fragmentation, inhibitors and sample handling errors during the extraction can be identified. The kinetic of fluoresce emission is proportional to the quantity of the synthesized DNA. Typical amplification curves have three phases. 
...
text(
    c(res_takeoff[[1]], res_takeoff_reverse[[1]]),
    c(res_takeoff[[2]], res_takeoff_reverse[[2]]) + c(0.95, -0.95), c("top", "tdp")
)
...
## Data Sets of Amplification Curves and Classification\label{section_data_sets}
...
**Note**: All users of the \texttt{PCRedux} package should use such tools before continuing to work with the records. Although most records in the \texttt{PCRedux} package have the same data structure, some records contain missing values or have different dimensions (compare data from \autoref{figure_sigmoid_cuve}). For example, the dataset `C127EGHP` spans a matrix of `r data_dim <- dim (chipPCR::C127EGHP); paste0(data_dim[1]," x ", data_dim[2])`, while the `htPCR` dataset comprises a matrix of `r data_dim <- dim (qpcR:: htPCR); paste0 (data_dim[1]," x ", data_dim[2])`.
...
The samples "A01", "A02", "A04" and "B04" from the ``C126EG685`` of the \texttt{chipPCR} package were analyzed.

```{r, eval=TRUE, echo=TRUE}
# Calculate curve features of an amplification curve dataset.
# Use the C126EG685 dataset from the chipPCR package and analyze the samples
# A01, A02, A04 and B05.
...
The results of the cluster analysis led two large clusters. A deeper inspection shows that the samples are correctly assigned to a cluster of positive or negative amplification curves. Moreover, the later increase of the fluorescence is reflected in the positive cluster (\autoref{qPCR2fdata}).
...
In this section, the methods of the ``pcrfit_single()`` function are described concisely and the underlying hypotheses are formulated. This methods to detect change points or to calculate slopes in specific regions of interest (ROI) of the amplification curve. The following overview gives some comments on the internal algorithms used by the ``pcrfit_single()`` function. The goal is not to examine the limits of their applicability, but rather to prove their basic functionality. 

The algorithms of the ``pcrfit_single()`` function use only certain ROIs to calculate specific features. In order to bring a systematic into the algorithms, the functions are summarized in groups in the following. Due to the number of algorithms, it is not possible to provide a detailed description with examples in every place. To underscore the usability of the algorithms and their features, selected amplification curve datasets (`batsch1`, `boggy`, `C126EG595`, `competimer`, `dil4reps94`, `guescini1`, `karlen1`, `lievens1`, `reps384`, `rutledge`, `testdat`, `vermeulen1`, `vermeulen1`) were analyzed with the ``encu()`` function and the results (features) were combined in the file `data_sample.rda`. The algorithms are divided into the following broad categories:
...
However, NAs pose a difficulty in many analyses. Before an analysis is carried out, it must be clarified how to deal with the missing values.
Under the term "imputation" there are a number of procedures based on statistical methods (e. g. neighboring median, spline interpolation) or on user-defined rules. Such a rule could, for example, consist of setting one of the slope parameters of a model to zero when it cannot be determined [@williams_rattle:_2009, @cook_interactive_2007, @hothorn_handbook_2014]. 
The application of fixed rules brings the advantage that the user is relieved of the decision as to how to deal with missing values. The disadvantage is that certain rules do not necessarily have to reflect a natural process.
...
The methods of the ``pcrfit_single()`` function are based on considerations and experience. Selected methods have been implemented as standalone functions to make them available to other applications. These functions are discussed in more detail in:

* \autoref{section_autocorrelation_test} for the ``autocorrelation_test()`` function, wich can be used to detect positive amplification curves by and autocorrelation analysis,
* \autoref{section_mblrr} for the ``mblrr()`` function, which is used to analyze the head and tail regaion by a Local Robust Regression,
* \autoref{section_hookreg} for the ``hookreg()`` and ``hookregNL()`` functions, which can be used to detect Hook effekt-like curvatures,
* \autoref{section_head2tailratio}  for the ``head2tailratio()`` function, which calculates the ratio between the head and the tail of an amplification curve and 
* \autoref{section_earlyreg} for the ``earlyreg()`` function, which calculates the Slope and Intercept of first six cycles of an amplification curve.
...
# Load example data (sample F6.1) from the testdat dataset
options(warn = -1)
library(qpcR)
library(magrittr)
...
In the literature, statistical methods are described which can be used to describe quantitatively the product formation in a qPCR. As illustrate in  \autoref{l5} are these the Ct-value, the maximum of the first derivative and the maximum of the second derivative [@roediger2015chippcr, @ruijter_evaluation_2013]. No further information shall be provided about these typical procedures. Another method is the *takeoff point* (`top`) according to @tichopad_standardized_2003. The `top` is calculated using externally studentized residuals, which tested to be an outlier in terms of the t-distribution. The `top` signifies to first PCR cycle entering the exponential phase. The `tdp` is an implementation in the ``pcrfit_single()`` function, which uses the inverse ordered amplification curve for calculation \autoref{amplification_curve_ROI}A describes the location of `top` and `tdp` exemplarily. The position (`f.top`, `f.tdp`) on the ordinate is also determined from these points. If an amplification curve is negative or neither `top` nor `tdp` can be calculated, then `top` & `tdp` will be assigned the number of cycles and `f.top`/`f.tdp` the value 1.
...
# Select randomly samples from 70% of the data for training.
# n_train is the number of samples used for training.

n_train <- round(nrow(data) * 0.7)

# index_test is the index of samples to be selected for the training
index_test <- sample(1:nrow(dat), size = n_train)

# index_test is the index of samples to be selected for the testing
...
```{r, echo=TRUE, fig.cap="\\label{earlyreg_results}Clusters of samples according to their slope and intercept. The `htPCR` dataset (\\texttt{qpcR} package) was used. The amplification curves show different slopes in the early phase (cycle 1 to 10) of the qPCR. Both the slope and the intercept were used for a cluster analysis (k-means).", fig.height=10, fig.width=10} 
...
pairs(res_earlyreg, col = cl[["cluster"]]) 
```
...
feature <- c("eff", "cpD1", "cpD2", "fluo", "init1", "init2", 
             ...
             "slope_background", "intercept_background", "MIC_background", 
             "polyarea", "changepoint.e.agglo", "changepoint.bcp", 
             "amptester_polygon", "amptester_slope.ratio", "minRFU", "maxRFU", 
             "bg.start_normalized", "bg.stop_normalized", "amp.stop_normalized", "amp_cor_MIC",
