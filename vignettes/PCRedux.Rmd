...
# Introduction to the Detection and Quantification of Nucleic Acids\label{label1}
...
For the generation of negative amplification curves, a set of experiments was conducted to amplify sequences of the Human papillomaviruses (HPV). The availability of negative amplification curves is limited, since in most cases only a few negative controls are included in qPCR experiments. All PCR conditions were taken from @senkel_entwicklung_2017. Biomarkers such as human papilloma virus (HPV) DNA was previously used in forensic applications[^1]. The findings and data set used in this study were made available as open data bundled in the open source software package called \texttt{PCRedux} (`r citation("PCRedux")$url`).

All statistical analysis was performed using \texttt{RKWard} [@roediger_rkward_2012] v. >= 0.7.0z+0.7.1+devel1. \texttt{RKWard} is an integrated development environment and graphical user interface for the \texttt{R} (v. `r getRversion()`) statistical computing language. It has been shown that \texttt{R} is a powerful environment for reproducible and transparent data analysis in a highly customizable cross-platform environment [@roediger2015r].
