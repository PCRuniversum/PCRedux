...
amp_data <- RAS002[, c("cyc", "A01_gDNA.._unkn_B.Globin", "H10_ntc_ntc_B.Globin")]
...
```{r, echo=FALSE, fig.cap="\\label{plot_dat_Cq}Distribution of Cq values of positive and negative amplification curves. All Cq values were determined from the maximum of the first derivative (cpD1) (A) and the minimum of the second derivative (cpD2) (B) from a multi-parametric model. Positive amplification curves heaped up in the range between 10 and 35 PCR cycles. This differs from the distribution of negative amplification curves.", fig.height=5}
library(PCRedux)
...
```{r, echo=FALSE, fig.cap="\\label{plot_dat_EffTop}A) `eff`, B) `sliwin`, C) `cpDdiff`, D) `loglin_slope`, E) `cpD2_range`, F) `top`, G) `f.top`, H) `tdp`, I) `f.tdp`, J) `bg.stop_norm`, K) `amp.stop_norm`, L) `convInfo_iteratons`", fig.height=11, fig.width=11}
library(PCRedux)
...
```{r, echo=FALSE, fig.cap="\\label{plot_bg_pt}A) `b_slope`, B) `f_intercept`, C) `minRFU`, D) `maxRFU`, E) `init2`, F) `fluo`, G) `slope_bg`, H) `intercept_bg`, I) `sd_bg`, J) `head2tail_ratio`.", fig.height=11, fig.width=11}
library(PCRedux)
...
```{r, echo = TRUE, fig.cap = "\\label{earlyreg_slopes}Analysis of the ground phase with the ``earlyreg()`` function. A) The amplification curves show different slopes and intercepts in the early ground phase (ROI: cycle 1 to 5) of the qPCR. Amplification curves (n = 192) from the `RAS002` dataset were used. B) Both the slope and the intercept were used for a cluster analysis (k-means, Hartigan-Wong algorithm, \\textit{k = 3}). The amplification curves were separated into three clusters dependent on their slope and intercept (colored in red, green, balck). ", fig.height=7}
options(warn = -1)
library(PCRedux)
...
mtext("A", cex = 1.2, side = 3, adj = 0, font = 2)
abline(v = c(1,5))
rect(21,3500,45,4700, col = "white", border = NA)
text(3, 3250, "ROI")

par(fig = c(0.55,1, 0.5, 0.9), new = TRUE) 
plot(res_earlyreg, col = cl[["cluster"]], pch = 19)
mtext("B    k-means with k = 3", cex = 1.2, side = 3, adj = 0, font = 2)
```
