...
The purpose of the \texttt{PCRedux} package is to provide a software for the analysis and numerical description of sigmoid curves. The descriptors (features) (\autoref{section_Functions_of_PCRedux}) can be used for further applications such as data mining, automatic classification (e.g., positive, negative) and machine learning. In the following chapters information is provided, which can be used for the analysis and numerical description of quantitative real-time PCR amplification curves.

One may argue that a classification into positive and negative amplification is also feasible using other analytical methods such as melting curve analysis or an electrokinetic separation. However, this is not always possible or desirable. For example, no melting curves cannot be obtained with Taqman probes. An electrokinetic separation often requires too much effort for qPCR with high sample throughput. Moreover, the mere classification into positive or negative is not necessarily the only aim of the \texttt{PCRedux} package. Instead, it is aimed that users and developers have tools to classify amplification curves automatically by any category conceivable, though the examples described in the following focus on the classification as positive of negative.
...
mtext(paste0("A     ", "Ct = ", signif(res_th.cyc[1, 1], 4)), cex = 1.25, side = 3, adj = 0, font = 2)
...
mtext(paste0("B     ", "Ct = ", signif(res_th.cyc[1, 1], 4)), cex = 1.25, side = 3, adj = 0, font = 2)
...
print(
  xtable(
    ...
    ), caption = "Results of the `htPCR` data set classification. All the 
    amplification curves of the `htPCR` dataset were classified as `negative`, 
`ambiguous` and `positive` by individuals in eight analysis cycles 
(`test.result.1` $\\ldots$ `test.result.8`). If an amplification curve has 
always been classified with the same class, the last column (`conformity`) shows 
`TRUE`. As an example, the table shows 25 amplification curves with consistent 
classes and 25 amplification curves with differing classes (`conformity = 
FALSE`).", 
...
```{r, echo = TRUE, fig.cap = "\\label{earlyreg_slopes}. Analysis of the ground phase by ordinary least squares Regression. A) The amplification curves show different slopes and basel-line levels in the early phase (cycle 1 to 10) of the qPCR. B) Both the slope and the intercept were used for a cluster analysis (k-means, Hartigan-Wong algorithm, \\textit{k = 4}). The amplification curves were separated into four clusters dependent on their slope and intercept (colored in red, green, blue black). Amplification curves (n = 147) from the `competimer` dataset (\\texttt{qpcR} package).", fig.height=4}
options(warn = -1)
library(PCRedux)
...
amp_data <- PCRedux::RAS002[, c(1, 151, 152)]
