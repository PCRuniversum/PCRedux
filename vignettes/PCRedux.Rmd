...
matplot(
  x = boggy[, 1], y = boggy[, -1], xlab = "Cycles", ylab = "RFU",
...
Data sets it is important to make sure that the volume of the data is representative. In this package a comprehensive number of amplification curves were classified by hand and processed with the algorithms described. Although, this dataset is fairly large in comparison to what existed before there is no numeric evidence how well it reflects amplification curves in general. In particular, not all datasets have comparable case numbers. For example, the `htPCR` dataset encompass in total `r ncol(qpcR::htPCR)-1` amplification curves, while the `C127EGHP` dataset encompass in total `r ncol(chipPCR::C127EGHP)-2` amplification curves. While most of the amplification curves of the `C127EGHP` dataset have a classical sigmod curve shape exhibit amplification curves from the `htPCR` dataset noisy curvatures with non-sigmoid shape. One may question if the larger dataset introduces a confirmation bias.
