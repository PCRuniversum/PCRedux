...
The distribution of `top`, `tdp`, `f.top` and `f.tdp` is shown in \autoref{plot_dat_1}I-L. This figure shows that a `top` value and a `tdp` value enable a qualitative classification of the amplification reaction. An interesting aspect is that the positive `f.top` values are markedly lower than the negative `f.top` values (\autoref{plot_dat_1}J). The same applies inversely to the `tdp` values (\autoref{plot_dat_1}L). In this way, amplification curves can be classified according to these values.
...
The aim was to predict, based on the ``polyarea`` feature, whether an amplification curve is positive or negative. The feature `polyarea` has been randomly selected from all features to show that the area under an amplification curve can be used to distinguish positive and negative amplification curves. The hypothesis is that positive amplification curves have a larger area than negative amplification curves. As shown in \autoref{plot_dat_3}C, there is a statistically significant difference between positive and negative amplification curves. Also shown are the values of another method to calculate the area under an amplification curve. This method is called `amptester_polygon` (\autoref{plot_dat_3}Ds). It is expected that this method would allow even more reliable predictions. However, this question is not to be dealt with in the following example.
...
feature <- c(
    "eff", "ressliwin", "convInfo_iteratons", "cpD1", 
    "cpD2", "cpDdiff", "diffQ2_slope", "diffQ2_Cq_range", 
    "top", "f.top", "tdp", "f.tdp", 
    "bg.stop_normalized", "amp.stop_normalized", "fluo", "init1", 
    "init2", "minRFU", "maxRFU", "sd_ground_phase", 
    "b_slope", "f_asymmetry", "slope_background", "intercept_background", 
    "amptester_slope.ratio", "head_to_tail_ratio", "polyarea", "amptester_polygon", 
    "changepoint.e.agglo", "changepoint.bcp", "mcaPeaks_minima_maxima_ratio"
)
...
```{r, echo=FALSE, fig.cap="\\label{plot_dat_2}Some text", fig.height=11, fig.width=11}
library(PCRedux)
