...
- `bg.stop` is the end of the ground phase estimated by the ``bg.max()`` function [@roediger2015chippcr].
- `amp.stop` is the end of the exponential phase estimated by the ``bg.max()`` function [@roediger2015chippcr].
...
```{r, echo=FALSE, fig.cap="\\label{plot_dat_EffTop}A) `eff`, B) `sliwin`, C) `cpDdiff`, D) `loglin_slope`, E) `cpD2_range`, F) `top`, G) `f.top`, H) `tdp`, I) `f.tdp`, J) `bg.stop`, K) `amp.stop`, L) `convInfo_iteratons`", fig.height=11, fig.width=11}
library(PCRedux)
