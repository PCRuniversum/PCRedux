...
dim(data_sample_subset_balanced) ## Observations    Features
```
...
For the comparison of features the data set was enlarged. Selection criteria for the data sets were comparatively less stringent.
...
dim(data_sample_subset) ## Observations    Features
```
...
The goal is to demonstrate the basic functionality of the algorithms for the feature calculation. Similar concepts are presented in groups.  The algorithms are divided into the following broad categories:
...
feature <- c("polyarea", "peaks_ratio", "cp_e.agglo",
...
In the following example, the ``randomForest()`` function from the \texttt{randomForest} package [@liaw_classification_2002] was used for the classification. The aim was to find the most important features for the classification of positive and negative amplification curves. 
As response vector ($y$) served `decision` with its possible states "positive" and "negative" (factor). The features 
