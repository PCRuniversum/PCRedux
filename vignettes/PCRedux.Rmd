...
In this section, the methods of the ``pcrfit_single()`` function are described concisely and the underlying hypotheses are formulated. The goal is not to examine the limits of their applicability, but rather to prove their basic functionality. The algorithms of the ``pcrfit_single()`` function are restricted to ROIs (\autoref{amplification_curve_ROI}) to calculate specific features. 

The methods of the ``pcrfit_single()`` function are based on considerations and experience. Selected methods have been implemented as standalone functions to make them available to other applications. The following output shows all features and their data type (*num*, numeric; *int*, integer; *Factor*, factor; *logi*, boolean) which are determined from the second amplification curve of the `RAS002` dataset with the ``pcrfit_single()`` function.
...
In order to bring a systematic into the algorithms, the functions with similar approaches are summarized in groups. Due to the number of algorithms, it is not possible to provide a detailed description with examples in every place. 
...
for(i in (1L:12) + 12*1) {
    ...
    mtext(paste0(LETTERS[i - shift], "   ", feature[i]), cex = 1.2, side = 3, 
...
mtext(LETTERS[i + 2], cex = 1.2, side = 3, adj = 0, font = 2)
}
```
