...
legend("top", legend = c("Negative", "Positive"), fill = colors, bty = "n")
mtext("B    Inverse", cex = 1.2, side = 3, adj = 0, font = 2)
```
...
Further features determined with the ``pcrfit_single()`` function are `eff`, `ressliwin`, 
`cpDdiff`, `cpDdiff`, `diffQ2_Cq_range`, `bg.stop_norm` and `amp.stop_norm`.

- `eff` is the optimized PCR efficiency found within a sliding window. A linear model of cycles versus log(Fluorescence) is fit within a sliding window (for details see ``sliwin()`` function from the \texttt{qpcR} package).
...
The aim was to predict, based on the ``polyarea`` feature, whether an amplification curve is positive or negative. The feature `polyarea` has been randomly selected from all features to show that the area under an amplification curve can be used to distinguish positive and negative amplification curves. The hypothesis is that positive amplification curves have a larger area than negative amplification curves. As shown in \autoref{plot_cp_area}C, there is a statistically significant difference between positive and negative amplification curves. Also shown are the values of another method to calculate the area under an amplification curve. This method is called `amptester_polygon` (\autoref{plot_cp_area}Ds). It is expected that this method would allow even more reliable predictions. However, this question is not to be dealt with in the following example.
