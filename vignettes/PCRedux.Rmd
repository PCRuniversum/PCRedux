...
Why is there is need for this software? One may argue that a classification into -  e.&nbsp;g., positive and negative amplification - is also feasible using bioanalytical methods such as melting curve analysis or an electrokinetic separation. However, this is not always possible or desirable. 

- For example, melting curves cannot be obtained with Taqman probes. An electrokinetic separation often requires too much effort for experiments with high sample throughput. A classification must also be carried out for both melting curve analysis and electrokinetic separation. 
- There are algortihms such as \texttt{linreg} [@ruijter_amplification_2009] that require information on whether an amplification curve is negative or positive for subsequent calculation.
...
Instead, characteristics of amplification curves and sigmoid functions that can be used for the statistical and analytical description are discussed (\autoref{section_analysis_functions}). Further chapters describe the implementation of the hypotheses in the \texttt{PCRedux} package. This includes technologies used for quality control of the \texttt{PCRedux} package. 
...
```{r, echo=FALSE, fig.cap="\\label{plot_dat_EffTop}Amplification curves from the datasets `stepone_std`, `RAS002`, `RAS003`, `lc96_bACTXY`, `C126EG595` and `dil4reps94` were analyzed with the ``encu()``` function. These datasets contain positive and negative amplification curves.  Furthermore, the meta dataset contains amplification curves that exhibit a hook effect or non-sigmoid shapes, for instance. All amplification curves are manually classified. Altogether 626 positive and 317 negative amplification curves were included in the analysis. A) `eff`, B) `sliwin`, C) `cpDdiff`, D) `loglin_slope`, E) `cpD2_range`, F) `top`, G) `f.top`, H) `tdp`, I) `f.tdp`, J) `bg.stop`, K) `amp.stop`, L) `convInfo_iteratons`", fig.height=11, fig.width=11}
library(PCRedux)
...
```{r, echo=FALSE, fig.cap="\\label{plot_cp_area}Amplification curves from the datasets `stepone_std`, `RAS002`, `RAS003`, `lc96_bACTXY`, `C126EG595` and `dil4reps94` were analyzed with the ``encu()``` function. These datasets contain positive and negative amplification curves.  Furthermore, the meta dataset contains amplification curves that exhibit a hook effect or non-sigmoid shapes, for instance. All amplification curves are manually classified. Altogether 626 positive and 317 negative amplification curves were included in the analysis. C) In energy agglomerative clustering, positive amplification curves had fewer change points than negative amplification curves. These two change point analyses generally separate positive and negative amplification curves. D) This analysis used the Bayesian change point analysis. Positive amplification curves appear to contain more change points than negative amplification curves. Nevertheless, there is an overlap between the positive and negative amplification curves in both methods. This can lead to false-positive or false-negative classifications.", fig.height=11, fig.width=11}
library(PCRedux)
