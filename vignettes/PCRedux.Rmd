...
```{r, echo=FALSE, results='hide',message=FALSE, fig.cap="Commonly used methods for the analysis of quantification points. A) Linear plot of an amplification curve with a typical sigmoid shape. The Grey horizontal line is the threshold as determined by the \\textit{68-95-99.7 rule} from the fluoresce emission of cycle 1 to 10. The red horizontal line is the user defined threshold in the log-linear range of the amplification curve. The Ct is calculated from the intersection of the horizontal line and a quadratic polynomial fitted in to the amplification curve (see @roediger2015chippcr for details). B) The Amplification curve plot with a logarithmic ordinate visualizes the linear phase. C) Analysis of the amplification curve by fitting with a five parameter model (black line) (\\autoref{l5}). The red line is the first derivate of the amplification curve, with the maximum at 17.59 cycles. The maximum is used in selected system as Cq value and refered to as first derivative maximum (`cpD1`). The green line is the derivative of the amplification curve, with the maximum at 15.68 cycles a minimum approximately at 19.5 cycles. The maximum is used in selected system as Cq value and refered to as second derivative maximum (`cpD2`). The blue line is the amplification efficiency that is estimated from the trajactory of the exponential region. The `Eff` value 1.795 means that the amplification efficiency is approximately 89%.\\label{figure_quntifcation_points}", fig.height=8, fig.width=8} 
options(warn = -1)
library(qpcR)
library(chipPCR)
library(magrittr)
...
feature <- c(
    ...
    "b_slope", "f_intercept", "slope_background", "intercept_background", 
...
```{r, echo=TRUE, fig.cap="\\label{earlyreg_slopes}. Analysis of the ground phase by ordinary least squares Regression. A) The amplification curves show different slopes and basel-line levels in the early phase (cycle 1 to 10) of the qPCR. B) Both the slope and the intercept were used for a cluster analysis (k-means, Hartigan-Wong algorithm, \\textit{k = 4}). The amplification curves were seperated into four clusters dependent on their slope and intercept (colored in red, green, blue black). Amplification curves (n = 147) from the `competimer` dataset (\\texttt{qpcR} package).", fig.height=4}
options(warn = -1)
library(PCRedux)
...
y_range <- log(range(data[range, -1]))
...
res_earlyreg <- do.call(rbind, lapply(2L:ncol(data), function(i) {
  earlyreg(x = data[, 1], y = data[, i], range = 10, normalize = TRUE)
}))
...
par(mfrow = c(2, 3), las = 0, bty = "o", oma = c(0, 0, 0, 0))

matplot(
  data[, 1], log(data[, -1]), col = 1, pch = 19, lty = 1, type = "l",
  xlab = "Cycle", ylab = "log(RFU)", main = ""
)
mtext("A", cex = 1.25, side = 3, adj = 0, font = 2)
...
matplot(data[range, 1], log(data[range, which(cl[["cluster"]] == i) + 1]), xlab = "Cycle", 
        ylab = "log(RFU)", main = "", ylim = c(y_range[1], y_range[2]),
        type = "l", lty = 1, lwd = 2, col = i)
mtext(LETTERS[i + 2], cex = 1.25, side = 3, adj = 0, font = 2)
}
```
...
As response vector ($y$) served `decision` with its possible states "positive" and "negative" (factor). The features `amptester_shap.noisy`, `amptester_lrt.test`, `amptester_rgt.dec`, `amptester_tht.dec`, `amptester_slt.dec`, `amptester_polygon` and `amptester_slope.ratio` served as a matrix of predictors describing the model to be adapted. The `stepone_std`, `RAS002`, `RAS003`, `lc96_bACTXY` datasets were used for the analysis. This dataset contains almost equal proportions of positive and negative amplification curves (\autoref{plot_Logistic_Regression}A). Prior to this, the amplification curves were analyzed with the ``encu()`` function (\autoref{section_pcrfit_single_pcrfit_parallel}) and stored in the `data_sample.rda` file to save computing time. The file is part of the \texttt{PCRedux} package.
...
dat <- data.frame(data[, c("amptester_shap.noisy", 
                           "amptester_lrt.test", 
                           "amptester_rgt.dec", 
                           "amptester_tht.dec", 
                           "amptester_slt.dec",
...
model_rf = randomForest(decision ~ ., data = train_data, ntree = 4000, importance = TRUE)
...
```{r, echo=TRUE, fig.cap="\\label{plot_random_forest}Random Forest.", fig.height=4}
par(mfrow = c(1,3))
