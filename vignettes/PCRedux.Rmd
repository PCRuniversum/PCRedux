...
amp_data <- data.frame(RDML$new(paste0(path.package("PCRedux"), "/", "RAS002.rdml"))$GetFData())
...
```{r, echo=TRUE, fig.cap="\\label{autocorrelation}Autocorrelation analysis for amplification curves of the `RAS002` data set (\\texttt{PCRedux} package). A) All amplification curves of the `RAS002` data set. B) Positive curves and negative curves as determined by the `autocorrelation_test()` and a human expert. C) Performance analysis by the ``performeR()`` function (see \\autoref{section_performeR} for details).", fig.height=5.5, fig.width=11}
