...
Since machine learning algorithm for the analysis of amplification curve data were not available in the literature, it was necessary to speculate which characteristics should be extracted by the processing algorithm and broken down into characteristic vectors. The number of proposed features that can be created with the algorithms of the \texttt{PCRedux} package was presumably the most extensive collection at the time of first release (summer 2017). Previously, only a few characteristics of amplification curves were described in the literature. Thus, it would be too few to use them extensively for machine learning with qPCR data. An application of those for machine learning could also not be found.
...
```{r, echo=FALSE, fig.cap="\\label{plot_cpa_RAS002} Comparison of the number of change points between positive and negative amplification curves. All amplification curves of the `RAS002` dataset were generated with the ``pcrfit_single()`` function. This function uses a Bayesian change point analysis and energy agglomerative clustering. A) Positive amplification curves appear to contain more change points than negative amplification curves. B) In energy agglomerative clustering, positive amplification curves had fewer change points than negative amplification curves. These two change point analyses generally separate positive and negative amplification curves. Nevertheless, there is an overlap between the positive and negative amplification curves in both methods. This can lead to false-positive or false-negative classifications.", fig.height=4}
# Analyze amplification curves from the RAS002 dataset for changepoints
options(warn = -1)

library(PCRedux)
data <- data_sample[data_sample$dataset == "RAS002", ]

par(mfrow = c(1, 2))
stripchart(data[["changepoint.bcp"]] ~ data[["decision"]], vertical = TRUE, method = "jitter", pch = 19, ylab = "Number of change points")
mtext("A    Bayesian", cex = 1.25, side = 3, adj = 0, font = 2)

stripchart(data[["changepoint.e.agglo"]] ~ data[["decision"]], vertical = TRUE, method = "jitter", pch = 19, ylab = "Number of change points")
mtext("B    Agglomerative", cex = 1.25, side = 3, adj = 0, font = 2)
```
\newpage
